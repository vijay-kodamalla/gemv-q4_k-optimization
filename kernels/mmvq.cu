...
#include "unary.cuh"
#include "vecdotq.cuh"
#include "custom_q4k.cu"
#include <cstdint>

typedef float (*vec_dot_q_cuda_t)(const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & kbx, const int & iqs);
// Declaration of our custom kernel (defined in custom_q4k.cu)
...
static void mul_mat_vec_q_switch_type(
	...
	case GGML_TYPE_Q4_K: {
            ...
            if (simple_ok && cc >= 80) {
                ...
                // Grid: Half the rows (each CTA does 2 rows)
                const int num_ctas = (nrows_x + ROWS_PER_CTA - 1) / ROWS_PER_CTA;
                dim3 grid(num_ctas, 1, 1);  // ← Should match upstream's 2048!
                dim3 block(32, NWARPS);
                
                // Smem: 2 tile buffers + warp partials
                size_t size_w = ROWS_PER_CTA * TILE_SIZE * sizeof(block_q4_K);
                size_t size_partials = NWARPS * ncols_dst * sizeof(float);
                const size_t smem_size = size_w + size_partials + 128;
                
                switch(ncols_dst) {
                    case 1:
                        custom_q4k_gemv<32, NWARPS, ROWS_PER_CTA, TILE_SIZE, 1>
                            <<<grid, block, smem_size, stream>>>(
                            vx, vy, dst, num_blocks_total, stride_col_y, stride_col_dst);
                        break;
                    case 2:
                        custom_q4k_gemv<32, NWARPS, ROWS_PER_CTA, TILE_SIZE, 2>
                            <<<grid, block, smem_size, stream>>>(
                            vx, vy, dst, num_blocks_total, stride_col_y, stride_col_dst);
                        break;
                    case 3:
                        custom_q4k_gemv<32, NWARPS, ROWS_PER_CTA, TILE_SIZE, 3>
                            <<<grid, block, smem_size, stream>>>(
                            vx, vy, dst, num_blocks_total, stride_col_y, stride_col_dst);
                        break;
                    case 4:
                        custom_q4k_gemv<32, NWARPS, ROWS_PER_CTA, TILE_SIZE, 4>
                            <<<grid, block, smem_size, stream>>>(
                            vx, vy, dst, num_blocks_total, stride_col_y, stride_col_dst);
                        break;
                }    
